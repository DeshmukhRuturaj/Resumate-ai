                ...
                if st.button("Search Jobs"):
                    with st.spinner('Searching for jobs...'):
                        ...
                        if job_listings and job_listings.get('data') and job_listings['data'].get('aggregateSalaryResponse'):
                            ...
                            st.success(f"Found salary data for {len(results)} companies")
                            ...
                            for job in results:
                                ...
                                st.markdown(f"""
                                    <div style="border: 2px solid #2196F3; padding: 10px; border-radius: 10px; margin-bottom: 10px;">
                                        <h4 style="color: #2196F3;">{job['jobTitle']['text']} at {company['name']}</h4>
                                        <p><strong>Company Rating:</strong> {company['ratings']['overallRating']}/5.0</p>
                                        <p><strong>Open Positions:</strong> {company['counts']['globalJobCount']['jobCount']}</p>
                                        <p><strong>Base Salary:</strong> ${base_pay:,.2f}/year</p>
                                        <p><strong>Total Compensation Range:</strong></p>
                                        <ul>
                                            <li>Median (50th percentile): ${total_pay[2]['value']:,.2f}</li>
                                            <li>Range: ${total_pay[0]['value']:,.2f} - ${total_pay[4]['value']:,.2f}</li>
                                        </ul>
                                        <img src="{company.get('squareLogoUrl', '')}" alt="Company Logo" style="max-width: 100px;">
                                    </div>
                                    """, unsafe_allow_html=True)
                                st.markdown("---")
                        else:
                            st.warning("No salary data found for the given job title.")
